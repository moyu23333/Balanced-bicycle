...
IFX_INTERRUPT(cc60_pit_ch0_isr, 0, CCU6_0_CH0_ISR_PRIORITY)
{
	...
	//陀螺仪积分
	if(huandao_gyro_flag==1)
	{
	    icm_gyro_z-=icm_gyro_z_err;
	    huandao_gyro_distance+=(float)icm_gyro_z*0.002;
	}
 //平衡控制
	balance_control();
	steering_balance(angle,angle_dot,expect_angle);
	steering_control(STEER_MOTOR_MID-steering_PID.out);
    ECPULSE2=Encorder_get(GPT12_T2);
    //车库编码器计步
    if(cheku_flag==1)
    {
        cheku_distance+=(float)ECPULSE2*0.002;
    }
    //后轮闭环
	if(Rear_Motor_FLAG)
	{
	    ECPULSE2=Low_pass_filter(ECPULSE2);//低通滤波
	    rear_temp_pwm=Rear_Motor_PI(ECPULSE2,-real_speed);
	}
	else rear_temp_pwm=0;
	motor_control(rear_temp_pwm);//闭环控制
